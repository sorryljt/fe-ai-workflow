...
- **验收标准**：
  - [x] 第 1 步末尾包含 git diff 命令和条件分支
  - [x] 专项提示列出：using-fe-workflow/SKILL.md、CLAUDE.md、AGENTS.md、.cursor/rules/workflow.mdc
  - [x] 非 git 项目场景有跳过说明（不报错）
- **依赖**：无
...
- **文件路径**：`skills/05-documentation/SKILL.md`
- **验收标准**：
  - [x] 第 3 步包含统计 docs/adrs/ 文件数的逻辑（排除 README.md）
  - [x] 编号格式为三位数（ADR-001、ADR-002 等）
  - [x] 包含替代流程交互提示（询问 → 定位旧文件 → 更新状态字段）
  - [x] 输入不存在编号时有错误提示和重试引导
  - [x] ADR 模板状态字段显示：`草稿 / 已接受 / 已废弃 / 已替代（见 ADR-XXX）  ← 四选一`
  - [x] ADR 模板标题注释说明编号由 DOCUMENT 自动填入
- **依赖**：T001（同文件，顺序修改）
...
- **文件路径**：`skills/05-documentation/SKILL.md`
- **验收标准**：
  - [x] 第 4 步包含 4 种变更类型 → 更新目标的对应规则表
  - [x] 包含"文件不存在时先创建骨架"的边界处理
  - [x] 包含 project-context.md 最后更新日期的同步说明
  - [x] 骨架创建前不需要用户干预（自动处理）
- **依赖**：T002（同文件，顺序修改）
...
- **验收标准**：
  - [x] 第 6 步在第 5 步之后，触发条件为"生成 project-context.md 完成后"
  - [x] 4 个骨架模板内容完整（表头、维护说明、示例行）
  - [x] 文件已存在时跳过并给出提示
  - [x] 验证标准 checklist 更新（新增 4 个活文档文件项）
  - [x] 导航卡无需变更（已有）
- **依赖**：无
...
- **验收标准**：
  - [x] 第 1 节：文件清单与职责（5 个文件、各自的更新触发节点和时机）
  - [x] 第 2 节：更新原则（追加不覆盖、Markdown 表格格式、每次 DOCUMENT 必须更新）
  - [x] 第 3 节：ADR 状态机制（4 个状态定义、变更时机、替代流程说明）
  - [x] 第 4 节：工作流自身变更同步规范（修改 skills/commands 时必须同步的文件清单）
  - [x] 第 5 节：活文档退化的识别与修复（何时需要重跑 /viktor:init）
- **依赖**：T002（状态机制内容参照）
...
- **文件路径**：`CLAUDE.md`
- **验收标准**：
  - [x] INIT 节点说明包含"生成活文档骨架"
  - [x] DOCUMENT 节点说明包含 ADR 自动编号、替代流程、活文档更新、变更检测
  - [x] 产物目录 `docs/` 章节包含 4 个新文件
...
- **验收标准**：
  - [x] INIT 节点说明更新
  - [x] DOCUMENT 节点说明更新（含 4 项新能力）
  - [x] 产物目录更新
  - [x] 与 CLAUDE.md 的节点说明逻辑一致（可简化，但不能矛盾）
...
- **验收标准**：
  - [x] INIT 描述包含"生成活文档骨架"
  - [x] DOCUMENT 描述包含 ADR 编号、替代流程、活文档更新
  - [x] 命令速查表和 Skill 对照无错误
- **依赖**：T006
...
- **验收标准**：
  - [x] doc.md description 包含"活文档更新"和"ADR 自动编号"
  - [x] init.md description 包含"活文档骨架"
- **依赖**：T009
...
- **文件路径**：
  ...
  - `CHANGELOG.md`
- **验收标准**：
  - [x] README.md 版本号 v0.4.0
  - [x] team-workflow-guide.md 版本号 v0.4.0
  - [x] CHANGELOG.md [Unreleased] 包含活文档体系、ADR 编号、替代流程、变更检测 4 条说明
- **依赖**：T010
...
- [x] 所有 P0 任务完成：DOCUMENT + INIT 两个 SKILL.md 改造完毕
- [x] `skills/05-documentation/SKILL.md` 包含 4 项新能力（变更检测/自动编号/替代/活文档更新）
- [x] `skills/06-project-init/SKILL.md` 包含第 6 步和 4 个骨架模板
- [x] `references/living-docs-conventions.md` 存在且包含 5 个章节
- [x] CLAUDE.md / AGENTS.md / workflow.mdc 三端描述一致
- [x] `skills/using-fe-workflow/SKILL.md` 已同步
