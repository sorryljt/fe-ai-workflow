...
- **验收标准**：
  ...
  - [x] 触发条件说明（命令触发 + 自然语言路由）
  - [x] 前置产物检查逻辑（tasks.md > design.md > 停止并引导）
  - [x] 执行步骤完整（检查前置 → 识别类型分组 → 生成 .types.ts → 自审 → 确认 → commit）
  - [x] 产物格式示例（TypeScript + JSDoc 分组注释）
  - [x] 自审规则（无 any / 无 TODO / 命名 PascalCase / 每个 export 有 JSDoc）
  - [x] 边界条件处理（重复触发时询问覆盖/追加；只有 design.md 时标注精度说明）
  - [x] 导航卡模板
  - [x] 验证标准 checklist
- **依赖**：无
...
- **验收标准**：
  - [x] `commands/viktor/contract.md` 包含：description frontmatter、加载 Skill 指令、前置检查说明、$ARGUMENTS 处理、完成后导航卡说明
  - [x] `.claude/commands/viktor/contract.md` 内容格式与其他 `.claude/commands/viktor/*.md` 一致
...
- **验收标准**：
  - [x] 第 5 步后新增推荐评估小节，包含推荐规则表格（6 条规则）
  - [x] 导航卡（推荐走 CONTRACT 时）包含两个选项 A/B，格式与设计文档一致
  - [x] 导航卡（建议跳过时）包含推荐 code + 可选 contract 两行
  - [x] 原有导航卡完整替换，无残留旧版本内容
- **依赖**：T001
...
- **验收标准**：
  - [x] 新步骤位于第 1 步之前（或作为第 0 步）
  - [x] 检查逻辑：存在 → 读取并在对话中标注；不存在 → 静默继续
  - [x] 标注文案与设计文档一致
  - [x] 不影响原有 TDD 循环步骤编号逻辑
- **依赖**：T001
...
- **验收标准**：
  - [x] 轴六标题与条件说明（仅合约文件存在时）
  - [x] 包含 4 条检查项（见设计文档 §4.5）
  - [x] 与现有检查轴格式一致
- **依赖**：T001
...
- **验收标准**：
  - [x] 命令速查表新增：`/viktor:contract` | CONTRACT | 触发时机说明
  - [x] Skill 映射表新增：CONTRACT → `skills/07-type-contract/SKILL.md`
  - [x] Codex 文本触发规则新增 `viktor:contract`
  - [x] 自然语言路由表新增对应意图示例
- **依赖**：T001
...
- **文件路径**：`CLAUDE.md`
- **验收标准**：
  - [x] 工作流图新增 `[/viktor:contract]` 节点（位于 plan 与 code 之间）
  - [x] 产物目录规范新增 `docs/contracts/` 说明
  - [x] 节点详细说明新增 CONTRACT 节点（触发方式、Skill 路径、输入、输出、完成条件）
  - [x] 节点顺序与 using-fe-workflow 保持一致
- **依赖**：T001
...
- **文件路径**：`AGENTS.md`
- **验收标准**：
  - [x] 工作流图与 CLAUDE.md 一致
  - [x] CONTRACT 节点说明存在（可适度精简，但逻辑不缺失）
  - [x] 命令触发规则包含 `viktor:contract`（无前导 `/`）
- **依赖**：T007
...
- **文件路径**：`README.md`
- **验收标准**：
  - [x] 版本号（快速开始章节中的 submodule 命令）更新为 v0.3.0
  - [x] 工作流说明图新增 `/viktor:contract`
  - [x] 命令表新增 CONTRACT 行
- **依赖**：T007
...
- [x] T001 ~ T002 完成（P0 核心，所有后续任务的基础）
- [x] T003 ~ T009 完成（P1 主要改动）
- [x] T010 ~ T011 完成（P2 同步更新）
- [x] 三端一致性核查：CLAUDE.md / AGENTS.md / workflow.mdc 工作流图、命令列表、节点说明内容一致
- [x] skills/using-fe-workflow/SKILL.md 中 CONTRACT 的触发条件与 skills/07-type-contract/SKILL.md 一致
- [x] git tag v0.3.0 打出前，README.md 和 team-workflow-guide.md 版本号已更新
