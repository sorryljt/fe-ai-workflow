...
扫描项目结构，生成或更新 `docs/project-context.md`。
这份文档是项目的「知识地图」，后续每次 BRAINSTORM 时直接读取，无需重新探索项目结构。支持幂等执行：重复运行时询问"重新扫描"还是"仅补全缺失骨架"，不会意外覆盖已有产物。
...
检查 `docs/project-context.md` 是否已存在：

**不存在**：直接继续，执行完整的第 1-6 步。

**已存在**：询问用户意图：

```
检测到 docs/project-context.md 已存在（生成于 YYYY-MM-DD）。

A) 重新扫描  → 重新执行第 1-5 步，更新项目知识地图（适用于项目经历较大重构后）
B) 仅补全骨架 → 跳过第 1-5 步，只执行第 6 步（适用于仅需补全缺失的活文档文件）

请选择（A / B）：
```

- **选 A（重新扫描）**：正常执行第 1-6 步，覆盖已有 `project-context.md`
- **选 B（仅补全骨架）**：跳过第 1-5 步，直接执行第 6 步；`project-context.md` 保持不变；活文档文件已存在的跳过，不存在的创建

> 两种操作均不会删除已有产物内容，第 6 步骨架文件已存在时统一跳过，**重复执行 `/viktor:init` 是安全的**。
