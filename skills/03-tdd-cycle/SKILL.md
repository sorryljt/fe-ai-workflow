...
| 类型 | 测试重点 | 测试工具 |
|------|---------|---------|
| 有状态组件 | 用户交互行为（点击、输入、选择） | 组件测试库 + userEvent（React: RTL）|
| 表单组件 | 提交、验证触发、错误展示 | 组件测试库 + userEvent（React: RTL）|
| 列表组件 | 渲染、空状态、加载状态 | 组件测试库（React: RTL）|
| 模态框/弹窗 | 打开/关闭、确认/取消回调 | 组件测试库 + userEvent（React: RTL）|
...
> 以下测试模板以 **React/Next.js + Vitest** 为参考实现，其他框架请按对应工具类比。
