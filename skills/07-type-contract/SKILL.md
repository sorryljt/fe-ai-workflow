...
检查当前对话历史中，是否存在 **ANALYZE 节点刚完成的信号**（导航卡中含有指向 `/viktor:contract` 的 `▶` 提示）：
...
1. 扫描 `docs/plans/` 下所有 `*tasks.md`：
   - **有多个文件** → 列出让用户选择：
     ...
     A. 2026-05-19 · session-aware-confirmation（9 个任务）
     B. 2026-05-18 · contract-node（12 个任务）

     请选择要生成合约的任务文件（输入 A / B / ...）：
     ```
   - **只有一个文件** → 直接告知「将使用 [文件名] 作为输入」，无需选择，继续执行
2. 用户选定后（或单文件直接使用时），继续执行前置产物检查（含已有合约文件的覆盖/追加确认）
